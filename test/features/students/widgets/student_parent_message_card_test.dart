...
void main() {
  testWidgets('switches parent message templates and copies short/full text', (
    tester,
  ) async {
    ...
    expect(find.text('进步反馈'), findsOneWidget);
    expect(find.text('练习提醒'), findsOneWidget);
    expect(find.text('续费沟通'), findsWidgets);
    expect(find.text('去记录缴费'), findsOneWidget);
    expect(find.textContaining('后续课次安排'), findsOneWidget);

    await tester.tap(find.text('练习提醒'));
    await tester.pumpAndSettle();
    expect(find.textContaining('练习起收笔和中宫'), findsOneWidget);

    await tester.tap(find.text('复制短信短版'));
    await tester.pumpAndSettle();
    expect(copiedTexts.last, '回家请继续练习起收笔和中宫。');

    await tester.tap(find.text('续费沟通').first);
    await tester.pumpAndSettle();
    await tester.tap(find.text('复制微信整段'));
    await tester.pumpAndSettle();
