...
void main() {
  group('ProgressAnalysisService', () {
    test('analyzeStudentProgress uses recent ten records in ascending order', () async {
      final gateway = _CapturingGateway(
        responseText: '{"overall_assessment":"ok","trend_analysis":"","strengths":"","areas_to_improve":"","teaching_suggestions":[]}',
      );
      ...
      expect(prompt.indexOf('2026-03-03'), lessThan(prompt.indexOf('2026-03-12')));
      expect(prompt, contains('"overall_assessment"'));
      expect(prompt, contains('"teaching_suggestions"'));
    });

    test('analyzeStudentProgress throws when no attendance records are available', () async {
  ...
  group('DataInsightService', () {
    test('analyzeBusinessData builds prompt from typed summary', () async {
      final gateway = _CapturingGateway(
        responseText: '{"summary":"healthy","revenue_insight":"steady","engagement_insight":"good","risk_alerts":["watch Bob"],"recommendations":["follow up"]}',
      );
